...
from .helper import generate_pdf_hash
...
class DocumentModalHandler:
    ...
    async def process_document(self, document_content):
        ...
        # Generate hash for the pdf document
        document_hash = generate_pdf_hash(pdf_content)

        # Check if the document already exists
        existing_document = await self.get_existing_document(document_hash)
        if existing_document:
            return await self.get_existing_document_info(existing_document)
            ...
            try:
                ...
                return len(chunks), summary, chunks, embeddings, metadata, document_hash
            finally:
    ...
    @sync_to_async
    def get_existing_document(self, document_hash):
        try:
            return DocumentMessage.objects.get(document_hash=document_hash)
        except DocumentMessage.DoesNotExist:
            return None

    @sync_to_async
    def get_existing_document_info(self, document):
        return (
            document.num_chunks,
            document.summary,
            None,  # We don't need to return chunks for an existing document
            None,  # We don't need to return embeddings for an existing document
            {
    ...
    @staticmethod
    async def save_document_message(message, document_data, num_chunks, summary, chunks, embeddings, metadata, document_hash):
        @sync_to_async
        def create_document_message():
            ...
            return DocumentMessage.objects.create(
                ...
                processed_content="" if chunks is None else " ".join(
